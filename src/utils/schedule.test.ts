...
describe("getEventPosition", () => {
  ...
  test("길이가 0인 일정도 최소 30분 높이를 갖는다", () => {
    ...
    const { height } = getEventPosition(sameMoment, sameMoment, dayStart);
...
describe("canEditSchedule (수정·삭제 권한)", () => {
  ...
  test("일반 멤버(MEMBER)는 남의 일정을 수정할 수 없다", () => {
    expect(
      canEditSchedule({ createdBy: 7, myMemberNo: 8, myGroupRole: "MEMBER" }),
    ).toBe(false);
  });
