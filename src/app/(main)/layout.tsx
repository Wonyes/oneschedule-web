    ...
    <div className="flex flex-col w-full h-full overflow-hidden">
      ...
      <div className="relative flex-1 flex overflow-hidden sm:pt-4">
        <main className="flex w-full h-full p-3 gap-3 overflow-hidden sm:p-4 sm:gap-6">
          <Sidebar />
          <div className="scroll-stable flex-1 min-w-0 min-h-0 h-full overflow-y-auto overscroll-contain px-1 pt-1 pb-[calc(4.5rem+env(safe-area-inset-bottom))] sm:pb-0">
            {children}
          </div>
          <GlobalOverlays />
          {isLoggedIn && <Sheet />}
