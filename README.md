...
| **컴포넌트**   | 가입 동의(전체/개별 토글), 종일 줄(다일 막대가 걸친 날짜 수만큼 이어지는지), 이메일 인증 스텝(쿨다운 · 만료 · 완료 상태)              |
...
| 🧪 **컴포넌트 테스트**  | 남은 것 — 캘린더 뷰(주/월 렌더링)와 시트 폼 동작                                                 |
| ♿ **접근성 심화**      | 캘린더 그리드 키보드 탐색(방향키), 스크린리더용 날짜 레이블                                      |
| 🎫 **그룹 초대 링크**   | `/join/{code}` 공개 페이지 · 동적 OG · 로그인 후 복귀(Return URL). 구글 OAuth 경로는 쿠키가 필요 |
